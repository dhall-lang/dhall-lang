...
At a high level, resolving an import means:

1. Determine which resource the import refers to.
2. Read the data from that resource or from the cache if available. (But see below for imports `as Location` that don't actually require reading any external resources.)
3. Parse the fetched text data as a Dhall expression (unless it is a non-code import).
4. Code imports are Dhall expressions that could again contain imports. Recursively resolve any imports inside the fetched expression (unless the import mode does not require recursive resolution).
5. After recursively resolving all imports, the code typically needs to be typechecked.
6. Normalize the result (unless it is an import `as Source`).
7. Possibly compute the hash of the result and cache it on disk.
8. Return the final result to the parent expression.

The main complications come from:

- relative imports, which must be interpreted relative to the importing resource
- import cycles, which must be detected and rejected
- referential sanity and CORS checks for remote imports (see below)
- import alternatives (`e0 ? e1`) that require error handling
- integrity checks (`sha256:...`)
- standard-mandated caching behavior
- the distinction between ordinary imports and `as Source`
...
- `as Text`, `as Bytes`, and `as Location` do not need to run the ordinary code-import pipeline
...
The Dhall standard models this context as:

- the import stack: a non-empty ordered list of already visited imports, written `(Δ, here)`
- a map from imports to expressions, written `Γ`, encompassing imports that have been already fully resolved
...
- configuration for remote fetching and headers
- configuration for integrity-check caches

The current Haskell implementation stores more than this in a `Status` value.
For example, it also records a dependency graph. However, the dependency graph
is not required by the standard and is not fundamental to the import algorithm
itself.
...
We will discuss the operations of "canonicalization" and "chaining" in detail below.
...
The actual resource to fetch is `./schema/hobby.dhall`.
...
relative to the process working directory: `./lib/types.dhall`. The import stack becomes:

```text
[ ./lib/types.dhall, ./lib/util/other.dhall, ./program.dhall, fake-root-for-current-directory ]
```

The "chaining" operation computes `./lib/types.dhall` from `./lib/util/other.dhall` and `../types.dhall`.
...
the canonicalized path to that import is `https://server.com/util/other.dhall`.
...
Note that this check is made after chaining and canonicalization. So, a parent import `https://example.com/A.dhall` may contain the expression `./local-secret.dhall` but the actual child import will be `https://example.com/local-secret.dhall` rather than a local file.
...
As we read `./bar.dhall`, we find that we have to resolve `./foo.dhall` again. We start doing that, and find that the import stack already contains the same import `./foo.dhall`. In this way, we detect an import cycle (always a fatal error).

## Step 3: fetch according to the import mode

When the checks succeed, now the resolver behaves differently depending on the mode.
...
3. return a Dhall value describing the location

This is why an import `as Location` does not even require the resource to exist and does not attempt to read it.
...
- recursively walks child imports
...
If the bytes are found and their SHA-256 matches the requested hash, the import
is accepted without refetching the original resource.

### Case C: resolve from scratch

If the import was not already available from the caches, then the resolver:
...
With ordinary imports, every import boundary can inline a large amount
of structure into normal form.

With `as Source`, the cached artifact can instead look more like:
...
| Failure mode | Soft or hard for `?` | Explanation |
|---|---|---|
| Missing file | Soft | The resource is absent. |
| Missing environment variable | Soft | The resource is absent. |
| Unreachable URL / retrieval failure | Soft | The resource is absent for resolution purposes. |
...
| Parse error in fetched Dhall code | Hard | The resource is present but invalid. |
| Type error in fetched Dhall code | Hard | The resource is present but invalid. |
| Integrity check failure | Hard | The resource is present but does not match the promised content. |
| Cyclic import | Hard | This is a structural error, not an absence. |
| Referential sanity violation | Hard | This is a policy violation, not an absence. |
| CORS violation | Hard | This is a policy violation, not an absence. |

The rule of thumb is:

- "resource absent" is soft
- "resource present but bad" is hard
