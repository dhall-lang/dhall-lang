...
For efficiency, Dhall implementations are encouraged to optimize the evaluation of `Natural/fold n B g b` via a loop that computes the normal form of `b`, the normal form of  `g b`, the normal form of `g (g b)`, etc., and stop the evaluation if the normal form stops changing (even if the limit `n` has not been reached).
